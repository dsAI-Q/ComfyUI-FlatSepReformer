...
> 节点按三级策略自动兜底：
> 1. **混合语音路检测**：对两路做 VAD 切分 → 话语段谱质心聚类（男低女高，
>    边界段用基频裁决）→ 目标性别话语段拼回时间轴；
> 2. **窗口分离兜底**：全局分离"疑似失败"（某路几乎没有语音段=背景路，或
>    某路语音质量明显偏低=模型把两人混成两份）时，改按 **4 秒短窗口独立分离**
>    （短窗口更接近模型训练分布，常能正确分开）→ 每窗口按谱质心中位判定
>    性别 → 目标性别窗口段拼接回时间轴；
> 3. **正常分离回退**：两路都有足量清晰语音（模型本来就分开了）时，回退原有
>    gate/排序/repair 逻辑，**旧工作流行为完全不变**。
>
> 未检测到混合语音路时自动回退正常分离
...
若全局分离明显失败（一路是背景音、或两路都是低质混合内容），还会自动改用
**4 秒短窗口分离兜底**（短窗口下模型常能正确分开）。检测不到异常时自动回退
正常分离逻辑，不影响原结果。

**Q: 分离出来的声音太小，怎么调大？**
分离会把能量分给两路、且模型按 0.5 峰值归一化，单路音量通常低于原混音。节点
自带 **`output_gain`** 参数（默认 1.0）：调大到 **1.5~2.0** 即可明显提升音量
